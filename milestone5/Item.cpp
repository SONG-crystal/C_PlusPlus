...
namespace sdds {
  ...
  Item::Item()
   ...
   {
      return strcmp(m_Name, rightItem.m_Name) > 0;
   }
   ...
   Item& Item::addQuantity(int value)
   {
      m_Quantity += value;
      return *this;
   }
    ...
    ostream& Item::write(ostream& Cout) const
   {
      ...
      else
      {
         ...
         { //POS_FORM
            ...
            Cout.width(13);
            if (m_Taxed) {
               Cout << std::left << "Price + tax: " << std::fixed<<setprecision(2)<< m_Price * (1 + TAX) << endl;
            }
            else {
